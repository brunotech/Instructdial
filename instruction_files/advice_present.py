...
instruction_dict = {
    ...
    "Positive Examples": [
        {
            "text": f"{settings.CONTEXT_SEP} Anyone take mental health days from work?. Do you use vacation, PTO, sick, FMLA? {settings.EOD_SEP} {settings.RESPONSE_SEP} Back at my old job, I used sick leave. {settings.QUESTION_SEP} Does the response provide advice for the issue in the text? Answer choices [OPTIONS] yes||||no ",
            "outputs": ['Yes'],
            "explanation": "",
        }
    ],
    "Negative Examples": [
        {
            ...
            + " I don\'t plan on seeing the solar eclipse but I just wanted to know why it\'s dangerous to look at it even though most of the sun is covered. "
            + settings.EOD_SEP
            + " "
            + settings.RESPONSE_SEP
            + " Back at my old job, I used sick leave. "
            + settings.QUESTION_SEP
            + " Does the response provide advice for the issue in the text? Answer choices [OPTIONS] yes||||no ",
            "outputs": ["No"],
            "explanation": "",
        }
    ],
    ...
    def generate_data(self):
        ...
        for d, dataset_reader in enumerate(self.data_readers):
            ...
            for dp in tqdm(datapoints):
                ...
                post_prompts = [
                    f"{settings.QUESTION_SEP} Does the response provide advice for the issue in the text? ",
                    f"{settings.QUESTION_SEP} Is the response appropriate? ",
                    f"{settings.QUESTION_SEP} Is the response helpful? ",
                    f"{settings.QUESTION_SEP} Does the response help resolve the issue raised in the text?",
                ]
                ...
                context = f'{context} {settings.RESPONSE_SEP} {advice}'
                ...
                context = f'{context} {settings.RESPONSE_SEP} {random_advice}'
                context_str = ' '.join(context.split()[-settings.MAX_DIALOGUE_LENGTH:])
