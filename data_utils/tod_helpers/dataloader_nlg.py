...
class Dataset_nlg(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        response_plain = f"{self.sys_token} " + self.data["turn_sys"][index]
        response = self.preprocess(response_plain)[:self.max_sys_resp_len]
        response_plain_delex = (
            f"{self.sys_token} " + self.data["turn_sys_delex"][index]
        )
        response_delex = self.preprocess(response_plain_delex)
        utterance_plain = f"{self.usr_token} " + self.data["turn_usr"][index]
        utterance = self.preprocess(utterance_plain)
        utterance_plain_delex = (
            f"{self.usr_token} " + self.data["turn_usr_delex"][index]
        )
        ...
        if self.args["nb_neg_sample_rs"] != 0 and self.mode == "train":
            ...
            for neg_resp in sampled_neg_resps:
                neg_resp_plain = f"{self.sys_token} {neg_resp}"
...
def collate_fn_nlg_turn(data):
    ...
    if "neg_resp_idx_arr" in item_info:
        neg_resp_idx_arr = []
