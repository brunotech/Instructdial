...
class Dataset_nlu(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args["example_type"] == "turn":
            context_plain = f'{self.start_token} {self.sys_token} {self.data["turn_sys"][index]} {self.usr_token} {self.data["turn_usr"][index]}'

            context = self.preprocess(context_plain)
            intent_plain = self.data["intent"][index]

            turn_sys_plain = f'{self.sys_token} {self.data["turn_sys"][index]}'
