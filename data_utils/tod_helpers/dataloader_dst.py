...
class Dataset_dst(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args["example_type"] == "turn":
            ...
            triggered_domains = {
                domain_slot.split("-")[0]
                for domain_slot in self.data["belief"][index].keys()
            }
            triggered_domains.add(self.data["turn_domain"][index])
            assert triggered_domains
    ...
    def concat_dh_sys_usr(self, dialog_history, sys, usr):
        return (
            f"{dialog_history} {self.sep_token} "
            + f" {self.sys_token} "
            + sys
            + f" {self.usr_token} "
            + usr
    ...
    def preprocess_slot(self, sequence):
        ...
        for value in sequence:
            v = list(self.tokenizer.encode(f"{value} {self.sep_token}"))
            story.append(v)
        return story
