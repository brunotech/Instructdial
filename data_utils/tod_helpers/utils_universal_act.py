...
def prepare_data_universal_act_dstc2(args):
    ...
    pair_trn = globals()[f"read_langs_{_example_type}"](file_trn, max_line)
    pair_dev = globals()[f"read_langs_{_example_type}"](file_dev, max_line)
    pair_tst = globals()[f"read_langs_{_example_type}"](file_tst, max_line)

    print(f"Read {len(pair_trn)} pairs train from {file_trn}")
    print(f"Read {len(pair_dev)} pairs valid from {file_dev}")
    print(f"Read {len(pair_tst)} pairs test from  {file_tst}") 
